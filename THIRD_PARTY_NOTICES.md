...
Nerfify uses the Rust crates and npm packages declared in `src-tauri/Cargo.toml` and `package.json`. Their licenses are resolved through their respective package metadata at build time; no signing credentials, private package registries, or generated secrets are committed.
