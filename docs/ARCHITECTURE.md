...
- `src-tauri/src/parser.rs` reads newline-terminated JSONL records, tolerates partial final lines, normalizes token counters, and emits fingerprints rather than raw content.
...
- `src-tauri/src/storage.rs` owns transactional schema creation, WAL, foreign keys, owner-restricted files, migrations, recovery copies, and DTO query projections.
...
The database stores accounts, source checkpoints, usage events, pricing snapshots, quota snapshots, epochs, measurements, quotes, annotations, heartbeats, settings, diagnostics, and app-run boundaries. A future ingestion worker should submit all estimator events through one serialized writer queue. No table stores prompts, raw account identifiers, or raw JSONL lines.
