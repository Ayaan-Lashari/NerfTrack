...
Run the commands in `docs/DEVELOPMENT.md` from the repository root. The GitHub Actions matrix performs the frontend gates, Rust gates, native build, and unsigned packaging smoke check on macOS ARM64, macOS Intel x86_64, Windows Intel x86_64, and the experimental Windows ARM64 preview runner. Windows ARM64 is non-blocking until the hosted runner is stable and available; Linux packaging and CI are intentionally excluded.

Unsigned artifacts, local databases, audit snapshots, dependency directories, and runtime output are ignored and must not be published. A project license remains a maintainer decision; no `LICENSE` file is created by this patch.
